...
export const VOLUME_OPTION_ID = "volume";
export const GIFT_WRAPPING_OPTION_ID = "giftWrapping";
...
export function getSelectedVolumePrice(
  volumes: { label: string; price: number }[],
  selectedVolume: string | undefined,
  fallbackPrice: number,
): number {
  return (
    volumes.find((volume) => volume.label === selectedVolume)?.price ??
    fallbackPrice
  );
}
